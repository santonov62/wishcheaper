...
const SHOP_NAME = 'techport.ru';
const SHOP_TITLE = 'Techport';

const log = (text, params = '') => {
  console.log(`[techportChecker.service] -> ${text}`, params);
};
...
const parse = async (url) => {
  ...
  try {
    ...
    log(`oldPrice`);
    try {
      oldPrice = await page.$eval('.tcp-product-body__price_column > .tcp-product-body__old-price', node => parseInt(node.innerText.replace(/\s/g, '')));
    } catch (e) { }
    
    log(`logo`);
    try {
      logo = await page.$eval('.slick-current > .product_image', node => node.getAttribute('src'))
    } catch (e) { }
